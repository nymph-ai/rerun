...
/// Common surface for lazy stores that keep a manifest-backed virtual index and load chunks on
/// demand.
///
/// Upstream's [`LazyStore`] covers static RRD files through `re_log_encoding::ChunkProvider`.
/// The fork's nereid lineage also has [`crate::LazyChunkStore`], which can hot-swap manifests and
/// evict by viewer cursor. `re_server` only needs this object-safe subset, so it can hold either
/// implementation without losing upstream's no-cache RRD path or nereid's live-corpus path.
pub trait LazyStoreLike: Send + Sync + 'static {
    ...
    fn extract_properties(&self) -> Result<arrow::array::RecordBatch, ExtractPropertiesError>;
    ...
    fn load_all_chunks(&self) -> ChunkStoreResult<Vec<Arc<Chunk>>>;
...
impl LazyStoreLike for LazyStore {
    fn store_id(&self) -> &StoreId {
        Self::store_id(self)
    }
    ...
    fn manifest(&self) -> Arc<RrdManifest> {
        Arc::clone(Self::manifest(self))
    }

    fn raw_manifest(&self) -> Arc<RawRrdManifest> {
        Arc::clone(Self::raw_manifest(self))
    }
    ...
    fn load_all_chunks(&self) -> ChunkStoreResult<Vec<Arc<Chunk>>> {
        Self::load_all_chunks(self)
    }
}

impl<P: crate::ChunkProvider> LazyStoreLike for crate::LazyChunkStore<P> {
    fn store_id(&self) -> &StoreId {
        Self::store_id(self)
    }
